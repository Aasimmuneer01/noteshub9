        ...
        async function loadWebsiteControl() {
            try {
                ...
                document.getElementById('global-premium-status').textContent = 'Status: ' + (globalFreePremium ? 'ON' : 'OFF');
            } catch (e) {
                console.error('Load website control error:', e);
            }
        }

        document.getElementById('toggle-global-premium').addEventListener('click', async () => {
        ...
        if ('serviceWorker' in navigator) {
            window.addEventListener('load', () => {
                navigator.serviceWorker.register('/noteshub9/admin-sw.js').then(registration => {
